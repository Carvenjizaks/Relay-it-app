-- Relay-it App Database Schema - Fix Migration
-- Run this in your Supabase SQL Editor

-- First, drop the old contacts table if it exists with wrong schema
...
CREATE INDEX idx_contacts_referral_token ON contacts(referral_token);
CREATE INDEX idx_referral_links_token ON referral_links(token);
CREATE INDEX idx_referral_links_contact_id ON referral_links(contact_id);
...
CREATE INDEX idx_perks_campaign_id ON perks(campaign_id);
CREATE INDEX idx_perk_claims_perk_id ON perk_claims(perk_id);
CREATE INDEX idx_perk_claims_contact_id ON perk_claims(contact_id);
